...
func (r *RayLogsHandler) GetContent(clusterId string, fileName string) io.Reader {
	ctx := context.TODO()
	// Object keys are written by the collector under {OssRootDir}/{clusterId}/{fileName}
	// (see the writer path), so reads must reconstruct the same prefix. Mirrors the S3 backend.
	...
	if err != nil {
		...
		allFiles := r._listFiles(path.Dir(fullPath), "", false)
		found := false
